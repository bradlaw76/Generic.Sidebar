🚀 Generic Sidebar for Dynamics 365 — Release v1.0.5 The Generic Sidebar kit provides a flexible, table-driven side pane for Dynamics 365 Customer Service and other model-driven apps. Instead of writing custom HTML/JS each time, you configure a single Dataverse table row to control instructions, embeds, icons, and theming.
...
Create a Config Row Open the Generic Sidebar Configuration table (sidebar_genericsidebar) and create one record with sidebar_default = Yes.

Set Key Fields Fill in:
...
<iframe ...></iframe> (Copilot or Canvas App) Raw HTML
Example of HTML Configuration Import Solution Screenshot

Example Displayed to End User Import Solution Screenshot

Example of Copilot Studio Configuration Import Solution Screenshot

Example Displayed to End User Import Solution Screenshot

Help, Issue Reporting and Survey
Import Solution Screenshot
...
⚠️ Disclaimer This kit is provided as-is. It is intended primarily for demo / proof-of-concept purposes. Before production use, admins must acknowledge the disclaimer via the welcome banner (sets sidebar_acknowledged = Yes).