...
- Pane create/reuse and title synchronization
- Same-configuration repeat-navigation suppression
...
(function () {
  ...
  function shouldNavigate(pane, configId, loadedConfigId) {
    return Boolean(
      pane?.__isNewPane ||
      !configId ||
      !loadedConfigId ||
  ...
  async function openSidebar(executionContext) {
    try {
      ...
      if (!shouldNavigate(pane, cfg.configId, window.__sidebarLoadedConfigId)) {
  ...
  if (typeof window !== 'undefined') {
    ...
    window.__GenericSidebarPaneRuntime = {
      ensurePane,
      getConfig,
      getDefaultConfigRow,
      getEnv,
